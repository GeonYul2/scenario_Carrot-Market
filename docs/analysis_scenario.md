...
3. 고위험군 중 개인화 조건(지역 일치 + 시급 10% 상승 + 동일/유사 업종)을 충족한 유저만 실행 타겟으로 확정한다.
4. AB 성과와 가드레일은 동일 모집단(실행 타겟)에서 함께 비교한다.
5. 주요 메트릭은 `apply_rate`, `uplift_vs_control`, `chi-square p-value`, `notification_blocking_rate`다.
6. 코호트 리텐션은 보조 지표로 후행 관찰한다.
...
결과(실측):
- 고위험군 `2536` 중 실행 가능 매칭 유저 `2105`
- 총 매칭 공고 수: `28515`
- 매칭 유저 1인당 평균 공고 수: `13.547` (중앙값 `9`)
...
- 그룹 분포(실측):
  - Control `693`, A `715`, B `697`

## I. AB 결과 + 가드레일
AB 성과(실행 매칭 성립 유저 기준):
- apply_rate:
  - Control `17/693` = `0.024531`
  - A `36/715` = `0.050350` (`uplift=+0.025819`)
  - B `114/697` = `0.163558` (`uplift=+0.139027`)
- 카이제곱(3x2): `chi2=104.411488`, `df=2`, `p=2.125e-23`
