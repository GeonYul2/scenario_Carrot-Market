...
### 4) 운영 적용(롤아웃 %)
- 적용 대상: `실행 매칭 성립 유저(기준일 정의: n=2103)`와 동일한 조건의 신규 타겟 풀
- 단계적 확대:
  - Week1: 전체 타겟의 `10%`에 `B(시급 상승 강조)` 적용
  - Week2: 이상 징후 없으면 `30%`로 확대
  - Week3+: 가드레일 충족 시 `100%` 전환(기본 운영안)

### 5) 중단(롤백) 기준
- 즉시 중단(Stop):
  - `notification_blocking_rate`가 `10.658%`(= Control `8.658%` + `2.0%p`) 초과
  - 또는 `apply_rate uplift vs Control`이 `+5.0%p` 미만으로 하락(2회 연속 관측)
- 중단 시 조치:
  - `B` 롤아웃 비율을 직전 단계로 롤백하고, 타겟 조건/빈도(캡) 재점검 후 재개
